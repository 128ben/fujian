...
const formState = reactive<FormState>({
  ...
  invite: userStore.invite || "k2qxF9",
  remember: true,
  satisfy: 1,
});
...
const onFinish = async (values: any) => {
  ...
  try {
    const res = await RegisterApi(submitObj);
    if (res.success) {
...
const urlInfo = ref(localStorage.getItem("urlInfo"))
const touristPlay = () => {
  
  if (userStore.invite) {
    if (urlInfo.value && urlInfo.value !== null) {
      window.open(urlInfo.value)
    } else {
      window.open(`https://www.worldballuniverse01.com/?invite=${userStore.invite}&url=https://www.worldballuniverse.com?invite=${userStore.invite}`)
    }
  } else {
    window.open("https://www.worldballuniverse01.com/?invite=k2qxF9&url=https://www.worldballuniverse.com?invite=k2qxF9")
  }
}
</script>
  ...
  <div class="loginForm">
    <a-form :model="formState" name="basic" autocomplete="off" @finish="onFinish" @finishFailed="onFinishFailed">
      ...
      <a-form-item label="username" name="username" :rules="[{ required: true, message: $t('login0.l31') }]">
        <div class="inputBox">
          <MobileOutlined />
          <a-input v-model:value="formState.username" type="number" :placeholder="$t('login0.l30')" />
        </div>
      </a-form-item>
      <!-- 密码 -->
      ...
      <a-form-item label="invite" name="invite" :rules="[
        { required: true, message: $t('login0.l25') },
      ]">
        <div class="inputBox">
          <ShareAltOutlined />
